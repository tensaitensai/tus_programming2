...
{
    int i;
    int num, a; //代入させたい配列番号、数値で使う
    build_heap(H, A, n);
    printf("代入させたい配列番号(%d~%d)を入力してください。\n", 0, n - 1);
    scanf("%d", &num);
    printf("配列に代入させる値(%d以上)を入力してください。\n", H[num]);
    scanf("%d", &a);
    increase_key(H, num, a);
    for (i = 1; i < n; i++)
    {
        A[n - i] = H[0];
        H[0] = H[n - i];
...
void increase_key(int *H, int i, int a) //課題2の指定された関数(insertと同じ)
{
    H[i] = a;
    upheap_sort(H, i);
}